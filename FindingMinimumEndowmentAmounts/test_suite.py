...
def test_sitemap_overlap():
    ...
    for entity_url in entity_urls:
        ...
        CandidateURLs, RedFlagURLs, VisitedURLs, *_ = process_entity(entity_url, names)
        candidate_set = set(c['url'] for c in CandidateURLs)
        redflag_set = set(r['url'] for r in RedFlagURLs)
        visited_set = set(v['url'] for v in VisitedURLs)
        # Check for RedFlag-violating URLs in CandidateURLs and VisitedURLs
        def is_redflag(url):
            from scraper import normalize_url, AllowedURLfileExtension, URL_Bad
            ...
            if ext and ext not in AllowedURLfileExtension:
                return True
            # domain mismatch and name checks omitted for test simplicity
            if any(bad in norm for bad in URL_Bad):
                return True
            return False
        # Fail if any RedFlag-violating URL is in CandidateURLs or VisitedURLs
        for url in candidate_set | visited_set:
            if is_redflag(url) and url not in redflag_set:
                print(f'[TEST FAIL] RedFlag-violating URL not in RedFlagURLs: {url}')
        # Fail if any duplicate URL is present
        normed_candidates = set()
        for c in CandidateURLs:
            norm = normalize_url(c['url'])
            if norm in normed_candidates:
                print(f'[TEST FAIL] Duplicate CandidateURL: {c["url"]}')
            normed_candidates.add(norm)
        normed_visited = set()
        for v in VisitedURLs:
            norm = normalize_url(v['url'])
            if norm in normed_visited:
                print(f'[TEST FAIL] Duplicate VisitedURL: {v["url"]}')
            normed_visited.add(norm)
